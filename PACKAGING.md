...
5. stage the GitHub release files, including `LICENSE` and `THIRD_PARTY_NOTICES.md`, in `ConditionalConfigSync.Plugin/package/github`;
...
Update `PluginInfo.PluginVersion` for a new release. `PluginInfo.PluginName` and `PluginInfo.PluginGuid` are the shared display name and BepInEx/Harmony identifier. The same metadata is used by the BepInEx plugin attribute and by both assembly information files. The packaging target reads the compiled plugin DLL version and writes it to `manifest.json`.
