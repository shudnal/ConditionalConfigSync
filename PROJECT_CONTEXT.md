...
37. Load an unchanged test consumer DLL compiled against the first public 1.x CCS API with the new core DLL; do not rebuild the consumer.
38. Compare the new public API against the retained 1.x baseline and investigate every removal or signature change.
39. Test a compatible older CCS client/server format whenever validation or package entry handling changes.
